...
import com.ayu.ml.cnn2.LayerWithKernel;
...
public class Cnn implements Serializable {
    ...
    public void train(DataSet dataSet, int trainCount) throws IOException {
        ...
        for (int i = 0; i < trainCount; i++) {
            ...
            logger.info("test precision ~2 " + testRight[0] + "/" + testCount[0] + "=" + testP);
            */
        }
    }
    ...
    public Integer predict(Record record) {
        forward(record);
        if (layers.size() > 0) {
            Layer lastLayer = layers.get(layers.size() - 1);
            if (lastLayer instanceof OutputLayer) {
                return ((OutputLayer) lastLayer).getLabel(record);
            }
        }
        return null;
    }
    ...
    private void updateParameters() {
        layers.stream().parallel().filter(layer -> layer instanceof LayerWithKernel)
                .forEach(layer -> {
    ...
    private boolean backPropagation(Record record) {
        ...
        layers.descendingIterator().forEachRemaining(layer -> {
            if (result[0] != null) {
            	layer.backPropagation(record);
            } else {
            	result[0] = layer.backPropagation(record);
            }
        });
        return result[0];
    }
