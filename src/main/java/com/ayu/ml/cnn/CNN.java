...
import com.ayu.ml.cnn.Dataset.Record;
import com.ayu.ml.cnn.ConcurenceRunner.TaskManager;
import com.ayu.ml.cnn.Log;
import com.ayu.ml.cnn.Utils;
import com.ayu.ml.cnn.Utils.Operator;
				...
				for (int j = start; j < end; j++) {
					for (int i = 0; i < lastMapNum; i++) {
						...
						for (int r = 0; r < batchSize; r++) {
							...
							if (deltaKernel == null)
								deltaKernel = Utils.convnValid(lastLayer.getMap(r, i), error);
							else {// 累积求和
								deltaKernel = Utils.matrixOp(Utils.convnValid(lastLayer.getMap(r, i), error), deltaKernel, null, null, Utils.plus);
							}
						}

						// 除以batchSize
						deltaKernel = Utils.matrixOp(deltaKernel, divide_batchSize);
						// 更新卷积核
						double[][] kernel = layer.getKernel(i, j);
						deltaKernel = Utils.matrixOp(kernel, deltaKernel, multiply_lambda, multiply_alpha, Utils.plus);
				...
				for (int i = start; i < end; i++) {
					...
					for (int j = 0; j < nextMapNum; j++) {
						...
						if (sum == null)
							...
							sum = Utils.matrixOp(Utils.convnFull(nextError, Utils.rot180(kernel)), sum, null, null, Utils.plus);
					}
		...
		new TaskManager(mapNum) {
			...
			public void process(int start, int end) {
				for (int m = start; m < end; m++) {
					...
					double[][] outMatrix = Utils.matrixOp(map, Utils.cloneMatrix(map), null, Utils.one_value, Utils.multiply);
					outMatrix = Utils.matrixOp(outMatrix, Utils.kronecker(nextError, scale), null, null, Utils.multiply);
