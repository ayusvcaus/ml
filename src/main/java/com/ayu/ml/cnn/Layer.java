...
//		double factor = 2 * Math.sqrt(6 / (fan_in + fan_out));
		kernel = new double[frontMapNum][outMapNum][kernelSize.x][kernelSize.y];
		for (int i = 0; i < frontMapNum; i++)
			for (int j = 0; j < outMapNum; j++)
				kernel[i][j] = Utils.randomMatrix(kernelSize.x, kernelSize.y,true);
	}
...
//		double factor = 2 * Math.sqrt(6 / (fan_in + fan_out));
		...
		for (int i = 0; i < frontMapNum; i++)
			for (int j = 0; j < outMapNum; j++)
				kernel[i][j] = Utils.randomMatrix(kernelSize.x, kernelSize.y,false);
	}
	...
	public void initBias(int frontMapNum) {
		this.bias = Utils.randomArray(outMapNum);
	}
